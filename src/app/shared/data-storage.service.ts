...
export class DataStorageService {
  ...
  getFiveDayForecast(city: string): Observable<RootObjectFiveDay> {
    return this.http.get<RootObjectFiveDay>(`${environment.openWeatherSpecificCityUrl}forecast?q=${city}&APPID=${environment.apiKey}`)
  }

  getCityWeather(city: String): Observable<RootObject> {
    return this.http.get<RootObject>(`${environment.openWeatherSpecificCityUrl}weather?q=${city}&APPID=${environment.apiKey}`)
  }
}
